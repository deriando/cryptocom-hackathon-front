...
class EventController{
    ...
    async setDDMFactory(this:EventController, provider=this.provider, caller= this.caller){
        if( this.DDMFactoryInstance !== null){
            const DirectDonationManagerFactoryInstance = new DirectDonationManagerFactoryInterface(caller as Signer,provider as providers.Provider);
            DirectDonationManagerFactoryInstance.setContract(process.env.GOERLI_PVT_KEY);
    ...
    async setDDManager(this:EventController ,managerAddress:string,provider=this.provider, caller= this.caller){
        if( this.DDManagerInstance !== null){
            const DirectDonationManagerInstance = new DirectDonationManagerInterface(caller as Signer,provider as providers.Provider);
    ...
    async setDirectDonation( donationAddress:string,provider=this.provider, caller= this.caller){
        if( this.DirectDonationInstance !== null){
            const DirectDonationInstance = new DirectDonationInterface(caller as Signer, provider as providers.Provider);
    ...
    async myDirectDonationManagerExist(): Promise<boolean>{
        return await (this.DDMFactoryInstance as DirectDonationManagerFactoryInterface).myDirectDonationManagerExist();
    }

    async getMyDirectDonationManager(){
        return await (this.DDMFactoryInstance as DirectDonationManagerFactoryInterface).getMyDirectDonationManager();
    }
    ...
    async createMyDirectDonationManager(){
        ...
        const tx = await (this.DDMFactoryInstance as DirectDonationManagerFactoryInterface).createMyDirectDonationManager();
